      SUBROUTINE RK2_TV_SOLVE(F, G, T0, TN, X0, N, Q, SEED)
C     *****************************************************************
C
C       RK2_TV_SOLVE solves a scalar stochastic differential equation
C       using a second-order time-varying Runge-Kutta method with
...
C
C         K1 = H * F(T1,X1) + H * G(T1,X1) * W1
C         K2 = H * F(T2,X2) + H * G(T2,X2) * W2
C
C         T2 = T1 + A21 * H
C         X2 = X1 + A21 * K1
C
...
C     f2py intent(out) X(0:N)
      ...
      T = T0
            ...
            CALL SDE_TABLE_HEADER('RK-2 Time-Variant')
